...
{
    min_sizes = pd.get(0, Tensor<float>());
    max_sizes = pd.get(1, Tensor<float>());
    aspect_ratios = pd.get(2, Tensor<float>());
...
int PriorBox::forward(const std::vector<Tensor<float>>& bottom_blobs, std::vector<Tensor<float>>& top_blobs) const
{
	/*
