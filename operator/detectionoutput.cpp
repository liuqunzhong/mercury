...
static inline float intersection_area(const BBoxRect& a, const BBoxRect& b)
{
	...
	float inter_width = (a.xmax < b.xmax? a.xmax : b.xmax) - (a.xmin > b.xmin? a.xmin : b.xmin);// std::min(a.xmax, b.xmax) - std::max(a.xmin, b.xmin);
	float inter_height = (a.ymax < b.ymax ? a.ymax : b.ymax) - (a.ymin > b.ymin ? a.ymin : b.ymin);// std::min(a.ymax, b.ymax) - std::max(a.ymin, b.ymin);
...
int DetectionOutput::forward(const std::vector<Tensor<float>>& bottom_blobs, std::vector<Tensor<float>>& top_blobs) const
{
	
    const Tensor<float>& location = bottom_blobs[0];
    const Tensor<float>& confidence = bottom_blobs[1];
    const Tensor<float>& priorbox = bottom_blobs[2];

    const int num_prior = priorbox.width() / 4;

    // apply location with priorbox
    Tensor<float> bboxes;
    bboxes.init(4, num_prior);
    if (bboxes.empty())
        return -100;
	/*
    const float* location_ptr = location.get_cpu_data();
