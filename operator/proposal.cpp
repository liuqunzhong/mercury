...
Proposal::Proposal()
{
    ...
    ratios.init(3);
	float* ptr_data = ratios.get_cpu_data_mutable();
	ptr_data[0] = 0.5f;
	ptr_data[1] = 1.f;
	ptr_data[2] = 2.f;

    scales.init(3);
	ptr_data = scales.get_cpu_data_mutable();
	ptr_data[0] = 8.f;
	ptr_data[1] = 16.f;
	ptr_data[2] = 32.f;
}

static Tensor<float> generate_anchors(int base_size, const Tensor<float>& ratios, const Tensor<float>& scales)
{
    int num_ratio = ratios.width();
    int num_scale = scales.width();

    Tensor<float> anchors;
    anchors.init(4, num_ratio * num_scale);
...
static inline float intersection_area(const Rect& a, const Rect& b)
{
    ...
    float inter_width = (a.x2 < b.x2? a.x2 : b.x2) - (a.x1 > b.x1? a.x1 : b.x1);//  std::min(a.x2, b.x2)std::max(a.x1, b.x1)
	float inter_height = (a.y2 < b.y2? a.y2 : b.y2) - (a.y1 > b.y1? a.y1 : b.y1);// std::min(a.y2, b.y2) - std::max(a.y1, b.y1);
...
int Proposal::forward(const std::vector<Tensor<float>>& bottom_blobs, std::vector<Tensor<float>>& top_blobs) const
{
    const Tensor<float>& score_blob = bottom_blobs[0];
    const Tensor<float>& bbox_blob = bottom_blobs[1];
    const Tensor<float>& im_info_blob = bottom_blobs[2];

    int w = score_blob.width();
    int h = score_blob.height();

    // generate proposals from bbox deltas and shifted anchors
    const int num_anchors = anchors.height();

    Tensor<float> proposals;
    proposals.init(4, w * h, num_anchors);
	/*
