...
int Net::forward_layer(int layer_index, std::vector<Tensor<float>>& blob_mats, bool lightmode) const
{
...
//     fprintf(stderr, "forward_layer %d %s\n", layer_index, layer->name.c_str());
    ...
    if (layer->one_blob_only)
    {
        ...
        Tensor<float> bottom_blob = blob_mats[bottom_blob_index];
        ...
        if (lightmode)
        {
            ...
            {
                bottom_blob.copyto(bottom_blob);
            }
        }
        ...
        {
            Tensor<float>& bottom_top_blob = bottom_blob;
    ...
    {
        // load bottom blobs
        std::vector<Tensor<float>> bottom_blobs;
        ...
        for (size_t i=0; i<layer->bottoms.size(); i++)
        {
            ...
            if (lightmode)
            {
                ...
                {
                    bottom_blobs[i].copyto(bottom_blobs[i]);
                }
            }
        }
        ...
        if (lightmode && layer->support_inplace)
        {
            std::vector<Tensor<float>>& bottom_top_blobs = bottom_blobs;
        ...
        {
            std::vector<Tensor<float>> top_blobs;
...
int Extractor::input(int blob_index, const Tensor<float>& in)
{
...
int Extractor::extract(int blob_index, Tensor<float>& feat)
{
    if (blob_index < 0 || blob_index >= (int)blob_mats.size())
        return -1;
...
#if USE_STRING
int Extractor::input(const char* blob_name, const Tensor<float>& in)
{
...
int Extractor::extract(const char* blob_name, Tensor<float>& feat)
{
    int blob_index = net->find_blob_index_by_name(blob_name);
    if (blob_index == -1)
        return -1;
